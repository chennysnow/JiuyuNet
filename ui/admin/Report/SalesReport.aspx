<%@ Page Language="C#" AutoEventWireup="true" CodeFile="SalesReport.aspx.cs" Inherits="admin_Report_SalesReport" %>
    ...
    <script type="text/javascript" >
        ...
        $(document).ready(function () {
            ...
            if (typeof (paraString) == "object" && paraString.length > 0) {
                ...
                $("#<%=dropTyp.ClientID%>").change(function () {

                    var cartgary = $("#<%=dropTyp.ClientID%>").val();
                    window.location.href = "?type=" + type + "&ctg=" + cartgary + "&repType=" + repType;
                });

                $("#repType").click(function () {
                    var cartgary = $("#<%=dropTyp.ClientID%>").val();
                    if ($("#repType").val() == "柱状图") {
                        window.location.href = "?type=" + type + "&ctg=" + cartgary + "&repType=1";
                        $("#repType").val("折线图");
                    }
                    else {
                        window.location.href = "?type=" + type + "&ctg=" + cartgary + "&repType=0";
                        $("#repType").val("柱状图");
                    }
...
<body>
    ...
    <ul >
     ...
     类别：<asp:DropDownList ID="dropTyp" runat="server"  Width="120px" ></asp:DropDownList>
    </div>
